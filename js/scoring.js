...
function calculateScore(levelNum, timeSec, jumpCount, totalRows, memTime, memTimeSaved, streakBonus, opts) {
  const { routeRevealed, totalCols, fakeChance, fakesRemaining } = opts || {};
  ...
  const timeScore = Math.max(0, SCORE_TIME_BASE - Math.floor(timeSec * SCORE_TIME_PENALTY))
                  + (fakesRemaining || 0) * SCORE_TIME_FAKE_BONUS;
...
function calculateStars(score, levelNum) {
  ...
  const totalNonPath = gridCells - cfg.rows;
  const maxFakeTimeBonus = Math.round(totalNonPath * fakePct) * SCORE_TIME_FAKE_BONUS;
  const maxScore = SCORE_TIME_BASE + maxFakeTimeBonus + SCORE_JUMP_BASE + levelNum * SCORE_LEVEL_MULT
